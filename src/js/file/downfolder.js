  ...
  async downloadFiles(url, fileName, controller, fileSize, updateProgress) {
    return new Promise((resolve, reject) => {
      ...
      xhr.onprogress = function (event) {
        if (!firstByteTime) {
          firstByteTime = performance.now(); // 首次接收到字节时记录时间
        }
      ...
      xhr.onload = function () {
        if (xhr.status === 200) {
          ...
          const firstByteTimeInt = parseInt(firstByteTime - startTime, 10); // 强制转换为整数
          console.log(
            `Time to first byte (TTFB) for ${fileName}: ${firstByteTimeInt} ms`
          );

          resolve({
            code: 0,
            msg: "Download successful",
            firstByteTime: firstByteTimeInt, // 将 TTFB 转为整数
          }); // 返回成功结果
      ...
      startTime = performance.now(); // 记录请求开始的时间
      xhr.send(); // 发送请求
    });
  }
      ...
      while (attempts < 3 && !successfulDownload) {
        ...
        try {
          ...
          uploadResults.push({
            ...
            fastestTime: result.firstByteTime ?? 0, // 记录 TTFB
          });
